...
class andirApiController {
    ...
    static async andirAppPlugin(ctx, next) {
        ...
        let { version, channl, systemVer, projectId, pluginName} = ctx.query;

        projectId = projectId || 1;
        pluginName = pluginName || '';

        let data = await new editMysql().getPlugAnListInfoData(version, channl, systemVer, projectId, pluginName);
